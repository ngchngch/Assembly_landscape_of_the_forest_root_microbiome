...
#########################################################################
save.dir <- "Output/03_08_Zland_abundance_occurence_250507"
dir.create(save.dir)
...
for(i in 1:length(zval_l)){#i <- 2
  ...
  for(k in 1:length(unique(lef$fb))){#i<- 1
    for(j in 1:length(unique(lef$plant))){#j<-1
      ...
      sel_lef <- lef[which(lef$fb==unique(lef$fb)[k]&lef$plant==unique(lef$plant)[j]&lef$p_even_BH<0.025&lef$p_land_BH<0.05),]
      
        th <- 2
        sel_lef1 <- sel_lef[which(sel_lef$z_even>0),]
        if(length(sel_lef1$z_even)>1){
          
        sel_tag1 <-   sel_lef1[order(sel_lef1$z_even,decreasing = TRUE)[1:th],"target"]
        }else{
          sel_tag1 <-   sel_lef1[,"target"]
          topmat1 <-   rbind(topmat1,sel_lef1)
        }
        
        sel_lef2 <- sel_lef[which(sel_lef$z_even<0),]
        if(length(sel_lef2$z_even)>1){
          
          sel_tag2 <-   sel_lef2[order(sel_lef2$z_even)[1:th],"target"]
        }else{
          sel_tag2 <-   sel_lef2[,"target"]
          topmat2 <-   rbind(topmat2,sel_lef2)
        }
        sel_tag <- c(sel_tag1,sel_tag2)
...
########################################
 ...
 for(pl in 1:length(plants)){#pl <- 1
  ...
  g1 <- ggplot(glef,
    ...
    labs(y="Z-standardized \u0394*evenness*\n( Fungal community )",
  ...
  ggsave(sprintf("%s/z_even_vs_RA_nolab_All_%s_%s.pdf",save.dir,fb,ra),g1,h=14,w=5,
         device = cairo_pdf)
  
  ggsave(sprintf("%s/z_even_vs_RA_All_%s_%s.pdf",save.dir,fb,ra),g1+
  ...
  ggsave(plot=g_legend(g1+theme(legend.position="right")),
         sprintf("%s/legend_z_even_vs_RA_All_%s_%s.pdf",save.dir,fb,ra),width=6,height=9,
         device = cairo_pdf)
  
  g1 <- ggplot(glef,
               aes(x=occurence,y=z_even))+
    #geom_vline(xintercept = 0,linewidth=0.2,color="gray60")+
    geom_hline(yintercept = 0,linewidth=0.2,color="gray60")+
    # geom_vline(data = data.frame(plant2=sprintf("*%s*",
    #                                             inf_border_l[which(inf_border_l[,2]==ra),3]),
  ...
  ggsave(sprintf("%s/z_even_vs_Occ_nolab_All_%s_%s.pdf",save.dir,fb,ra),g1,
         h=6,w=8,
         device = cairo_pdf)
  
  ggsave(sprintf("%s/z_even_vs_Occ_All_%s_%s.pdf",save.dir,fb,ra),g1+
           geom_text_repel(data=function(x){x[x$Top,]},
                           min.segment.length = 0.1,
                           aes(label=target_label),size=3,
                           parse = TRUE,
  ...
  ggsave(plot=g_legend(g1+theme(legend.position="right")),
         sprintf("%s/legend_z_even_vs_Occ_All_%s_%s.pdf",save.dir,fb,ra),width=6,height=9,
         device = cairo_pdf)
