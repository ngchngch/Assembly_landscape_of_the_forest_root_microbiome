...
save.dir <- "Output/08_01_glmm_mycorrhizal_colonization_Netherway_260227"
dir.create(save.dir)

#######
## SourceData_08 can download from Netherway et al., 2024
df_f <- read.csv("SourceData_08/root_FungiOTU_matrix.csv",row.names = 1)
tx_f <- read.csv("SourceData_08/FungiOTU_taxonomy.csv",row.names = 1)
df_b <- read.csv("SourceData_08/root_BacteriaOTU_matrix.csv",row.names = 1)
tx_b <- read.csv("SourceData_08/BacteriaOTU_taxonomy.csv",row.names = 1)

info <- read.csv("SourceData_08/env_metadata.csv",row.names = 1)
info$Row.names <- rownames(info)
#######
dir_03_02 <- "Output_supercomputer/03_02_Zconv_randELA_withRA_fixP"
# read data
...
for(i in 1:ntarget){
  ...
  model_ecm <- glmmTMB::glmmTMB(
    cbind(round(EcM_colonisation), total - round(EcM_colonisation)) ~ Tree_species+scale(Soil_pH)+scale(abundance) + (1 | Site),
    family = binomial(link = "logit"),
    data = merge_df2
  )
...
ord <- (scale(res[res$Var=="AM_colonization","Estimate"])+scale(res[res$Var=="EcM_colonization","Estimate"]) )%>%
  order(decreasing = TRUE)
res$target2 <- factor(sprintf("*%s*",res$target),
                     levels=sprintf("*%s*",res$target[res$Var=="AM_colonization"][ord]))
