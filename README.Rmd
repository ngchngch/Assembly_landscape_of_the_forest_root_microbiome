...
```{r}
mat0 = readRDS(sprintf("%s/02_PA_data.rds", workflow_path))
...
mdp <- foreach(sp = 1:80, .packages = c("rELA", "doParallel", "tidyverse")) %dopar% {
...
for (sp in 1:80) {
  ...
  mdp_rand <- foreach(rand = 1:nrandamization, .packages = c("rELA", "doParallel", "tidyverse")) %dopar% {
    bin0 <- as.matrix(mat2[, -sp])
    bin <- bin0[, colMeans(bin0) > 0.1 & colMeans(bin0) < 0.9]
    res <- Quant_landshift(
      bin = bin, # Binary community matrix (samples x species)
...
for (pl in 1:nrow(pltab)) { # pl <- 4
  showDG_mod(ela[[pl]], om,
    SS_colmat = as.data.frame(matrix(SStab[ss_pl, ],
      ncol = 3,
