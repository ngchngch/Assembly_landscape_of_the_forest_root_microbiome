...
#########################################################################
save.dir <- "03_01_03_ELA_withRA_4step_genuslevel"
dir.create(save.dir)
...
ocmat <- list(Fungi=readRDS(sprintf("%s/ELA_input_ocmat_Fungi.rds",dir_02_05_03)),
              Prokaryota=readRDS(sprintf("%s/ELA_input_ocmat_Prokaryote.rds",dir_02_05_03)))
...
#for(i in 1:length(list)){#i <-1
      ...
      f1 <- f/rowSums(f); f1[rowSums(f)==0,] <- 0
    ...
    b1 <- b/rowSums(b); b1[rowSums(b)==0,] <- 0
      b1 <- b1[which(rownames(b1) %in% rownames(ocmat$Prokaryota)),
      ...
      saveRDS(mdp,sprintf("%s/ELA_SSprob_diff_%s_%s.rds",part,fb,nam))
