...
#########################################################################
save.dir <- "03_01_04_ELA_withRA_4step_boot"
dir.create(save.dir)
...
#for(i in 1:length(list)){#i <-1
      ...
      rsample_seq <- sample(1:nrow(ocmatf0),replace = TRUE)
      ...
      which_pres0 <- Bin_2sd(ramat[,colSums(ramat>0)>30]);dimnames(which_pres0) <- dimnames(ramat[,colSums(ramat>0)>30])
      which_pres <- which_pres0[rownames(ocmatf0)[rsample_seq],]
      
      ocmatf <-ocmatf0[rsample_seq,]
      ...
      enmatf <- cbind(RA=scale(abmat[rownames(ocmatf0)[rsample_seq],nam]),
                      sp_info[rownames(ocmatf0)[rsample_seq],
                              -c(1,ncol(sp_info))])
      
      saveRDS(list(ocmatf,enmatf,which_pres,ramat[rownames(ocmatf0)[rsample_seq],nam],
                   info[rownames(ocmatf0)[rsample_seq],],rownames(ocmatf0)[rsample_seq]),sprintf("%s/inputdata_%s_wo%s_boot%s.rds",part,fb,nam,boot_id))
     ...
     mdp <- cbind(Taxa=nam,md_sprop)
      saveRDS(mdp,sprintf("%s/ELA_SSprob_diff_%s_%s_boot%s.rds",part,fb,nam,boot_id))
