...
#########################################################################
save.dir <- "05_02_ELA_withRA_multistep_eachseed_Nsp80_260201_othAB"
dir.create(save.dir)
...
for(conn in 1:length(cpath)){
  ...
  #if(sum(bin_sp==1)>10&sum(bin_sp==0)>10){
sa <- runSA(ocmat= bin,enmat = enmatf,
  ...
  ############
    ...
    ra_perc <- quantile(enmatf,qt_seq)
