...
#########################################################################
save.dir <- "03_03_ELA_withRA_4step_rep_241212"
dir.create(save.dir)
...
SSchange <- function(state,sa,env_grad,env_cat=NA,steps=4,eq_steps=TRUE,
  ...
  if(!is.na(de[1])){
    ...
    #delta evenness
    d_even <- c(res$h1[1]-res$h1[2:length(res$h1)])
...
#for(i in 1:length(list)){#i <-1
      ...
      #saveRDS(state, file=sprintf("%s/start_%s_%s.rds",part,fb,r))
      #######################
      ...
      for(pl in 1:nrow(unique(sp_info[,-1]))){#pl <-3
        ...
        if(sum(ra_noclr>0) > 4){
          ...
          if(!sprop[[pl]]$skip){
            ...
            sstabl[[pl]] <-NA
            md_sprop <- rbind(md_sprop,data.frame(plant=pl_nam,ab=c(NA,NA,NA),ra=c("perc25","median","perc75"),
      ...
      names(sstabl) <- pnam
      saveRDS(sstabl,sprintf("%s/SSprob_table_%s_%s_%s.rds",part,fb,nam,r))
      names(sprop) <- pnam
      saveRDS(sprop,sprintf("%s/SSprob_list_%s_%s_%s.rds",part,fb,nam,r))
      mdp <- cbind(Taxa=nam,md_sprop)
      saveRDS(mdp,sprintf("%s/ELA_SSprob_diff_%s_%s_%s.rds",part,fb,nam,r))
      #write.table(mdp,sprintf("%s/check_SSprob_diff_%s_%s_%s.txt",part,fb,nam,r))
