...
#########################################################################
save.dir <- "03_02_02_02_Zconv_ELA_withRA_4step_spatialblocking"
dir.create(save.dir)
...
list_or <- list.files(path=dir_03_01,"ELA_SSprob_diff", recursive = TRUE,full.names=TRUE)
rand_dir <- list.dirs(path=dir_03_02_02, recursive = FALSE)
...
p_deven <- ifelse(or_dle[,"d_even"]>0,
  ...
  res_df <- data.frame(ID=sp,fb=fb,taxa=tx,
                                    ...
                                    p_deven=p_deven)
  saveRDS(res_df,
          sprintf("%s/%s/%s/Zconv_ELA_results_spatial_blocking.rds",save.dir,fb,tx))
  ##########
  fb <- "Prokaryota"
  dir.create(sprintf("%s/%s",save.dir,fb))
