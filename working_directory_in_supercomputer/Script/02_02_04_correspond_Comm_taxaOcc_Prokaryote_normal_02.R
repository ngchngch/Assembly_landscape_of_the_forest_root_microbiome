set.seed(1234)
library(rELA)
library(vegan)

library(foreach)
library(doParallel)
library(tidyr)
library(compositions)
library(gtools)







source("packages/01_1_function.R")
#########################################################################
save.dir <- "02_02_04_correspond_Comm_taxaOcc_Prokaryote_normal_02"
dir.create(save.dir)
#####################
#ELA_prep_dir <- "/Users/ngch/Desktop/Now_using/data/sugaFB_ELA/analysis_series_SpCom/02_ELA_v060_241108/02_01_ELA_prep_abundance_threshold"
##select target taxa
unident_th <- 0.5

tb_m <- readRDS(sprintf("%s/Df_RA_Family.rds",ELA_prep_dir))

info <- readRDS(sprintf("%s/comp_sample_info_plant2.rds",ELA_prep_dir))
sp_info <- readRDS(sprintf("%s/ELA_input_plant.rds",ELA_prep_dir))

#######
fb <- "Prokaryota"
for(rel_th in seq(0,0.1,by=0.01)){
        dir.create(sprintf("%s/rel_th%s",save.dir,rel_th))
ocmat <- readRDS(sprintf("%s/ocmat_remove_M2SD_%s_Family_relth%s.rds",ELA_prep_dir,fb,rel_th))
or_ocmat <- readRDS(sprintf("%s/ocmat_remove_M2SD_%s_Family.rds",original_ELA_prep_dir,fb))

relf2 <- tb_m$Fungi/rowSums(tb_m$Fungi)

relf3 <- relf2[which(relf2[,"Unidentified"] < unident_th & rownames(relf2) %in% rownames(ocmat)),]


or_ocmat2 <- or_ocmat[rownames(relf3),]

or_ocmat3 <- or_ocmat2[,which(colSums(or_ocmat2)>10 & colSums(or_ocmat2) < nrow(or_ocmat2)-10)]

txnam <- colnames(or_ocmat3)[tx]

ocmat2 <- ocmat[rownames(relf3),]

ocmat3 <- ocmat2[,which(colSums(ocmat2)>10 & colSums(ocmat2) < nrow(ocmat2)-10)]

if(txnam %in% colnames(ocmat3)){
CTRL.t <- how(within = Within(type = "free"), 
              plots = Plots(type = "none"),
              blocks = paste0(info[rownames(ocmat3),c("plant")],
                              info[rownames(ocmat3),c("site")]), 
              nperm = 1000,
              observed = TRUE)

  exp_df <- data.frame(info[rownames(ocmat3),c("plant","site")],
                  Occ=ocmat3[,txnam])
  perm <- adonis2(data=exp_df,
          relf3~plant+site+Occ,
          by = "margin",
          permutations=CTRL.t,
          parallel = n.core)
  
  R2 <- data.frame(taxa=txnam,
                       R2=perm$R2[3],raw_p=perm$`Pr(>F)`[3])

saveRDS(R2,sprintf("%s/rel_th%s/R2_%s_%s.rds",save.dir,rel_th,colnames(ocmat3)[tx],fb))

}
}