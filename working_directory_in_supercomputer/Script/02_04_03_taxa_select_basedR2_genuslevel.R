...
#########################################################################
save.dir <- "02_04_03_taxa_select_basedR2_genuslevel"
dir.create(save.dir)
#####################
#parameters
taxa <- "Genus"
...
##select target taxa
tb_m <- readRDS(sprintf("%s/Df_RA_Genus.rds",ELA_prep_dir))

ocmat <- readRDS(sprintf("%s/ocmat_remove_M2SD_Fungi_Genus.rds",ELA_prep_dir))
...
##R2 of each taxa
df2 <- readRDS(sprintf("%s/Permanova_sig_taxa_R2.rds",dir_02_03_03))
#######
fb <- "Fungi"
...
ocmat <- readRDS(sprintf("%s/ocmat_remove_M2SD_Prokaryota_Genus.rds",ELA_prep_dir))
