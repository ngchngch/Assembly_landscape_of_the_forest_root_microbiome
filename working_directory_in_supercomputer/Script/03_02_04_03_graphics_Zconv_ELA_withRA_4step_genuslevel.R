...
#########################################################################
save.dir <- "03_02_04_03_graphics_Zconv_ELA_withRA_4step_genuslevel"
dir.create(save.dir)
...
z_val <- do.call(rbind,
        lapply(list.files(dir_03_02_04_02,"Zconv_ELA_results",full.names = TRUE,recursive = TRUE),
       readRDS))

z_val$p_land_BH <- p.adjust(z_val$p_dland,method = "BH")
z_val$p_even_BH <- p.adjust(z_val$p_deven,method = "BH")

zval_f_org <- z_val[z_val$fb=="Fungi",]
zval_p_org <- z_val[z_val$fb=="Prokaryota",]
  ...
  for(pl in 1:length(unique(zval1$plant))){
    for(f2 in 1:length(unique(zval1$target_k))){
      for(f in 1:length(unique(zval1$fb))){#pl <- 1;f <- 1
        ...
        top <- zval2[order(zval2$z_dland,decreasing = TRUE)[1:5],"target"]
...
le <- zval_l
  ...
  lef$z_dland[is.na(lef$z_dland)] <- 0
  lef$z_deven[is.na(lef$z_deven)] <- 0
  
  lef$landscape <- ifelse(lef$p_land_BH<0.05,
                          ifelse(lef$p_even_BH<0.025,
                                 ifelse(lef$z_deven>0,
                                        "Significant \u0394*topography* & \u0394*evenness* (increase)",
                                        "Significant \u0394*topography* & \u0394*evenness* (decrease)"),
                                 "Significant \u0394*topography*, non significant \u0394*evenness*"),
                          "Non significant \u0394*topography*")
  ...
  for(i in 1:length(unique(lef$fb))){#i<- 1
    for(j in 1:length(unique(lef$plant))){#j<-1
      for(ra in 1:length(unique(lef$ra))){#ra <- 1
        ...
        #   lef2 <- rbind(lef2,lef1[which.max(lef1$z_dland),])
        # }
        # 
        th <- 2
        
        if(nrow(lef2)>th){
          sel_tag <-   lef2[order(lef2$z_dland,decreasing = TRUE)[1:th],]
          
          topmat <-   rbind(topmat,lef2[order(lef2$z_dland,decreasing = TRUE)[1:th],])
        ...
        pos_lef <- sel_lef2[sel_lef2$z_deven>0,]
        neg_lef <- sel_lef2[sel_lef2$z_deven<0,]
       
        if(nrow(pos_lef)>th & nrow(neg_lef)>th){
          sel_tag2 <- rbind(pos_lef[order(pos_lef$z_deven,decreasing = TRUE)[1:th],],
                            neg_lef[order(neg_lef$z_deven)[1:th],])
         ...
         #topmat_e_upper <- rbind(topmat_e_upper,lef2[order(lef2$z_deven,decreasing = TRUE)[1:th],])
  ...
  for(plants in c("Acer","Betula","Pinus","Juglans","Populus","Larix")){
    a <- a+1
      g1[[a]] <- ggplot(lef[which(lef$fb==fb & lef$plant %in% plants & lef$ra %in% ra),],
       ...
       # geom_path(aes(group=Taxa),color="gray",
        #          arrow = arrow(length = unit(0.2, "cm")),
         #         linewidth=0.5)+
        geom_star(data=function(x){x[x$landscape=="Non significant \u0394*topography*",]},
                  aes(size=landscape,starshape=landscape,fill=target_p))+
        geom_star(data=function(x){x[x$landscape!="Non significant \u0394*topography*",]},
                  aes(size=landscape,starshape=landscape,fill=target_p))+
        labs(x="Z-standardized \u0394*topography* \n(Fungal assembly landscape)",
             y="Z-standardized \u0394*evenness* \n(Fungal assembly landscape)",
             fill="Genus",
        ...
        scale_size_manual(values = c("Non significant \u0394*topography*"=2,
                                     "Significant \u0394*topography*, non significant \u0394*evenness*"=4,
                                     "Significant \u0394*topography* & \u0394*evenness* (decrease)"=4,
                                     "Significant \u0394*topography* & \u0394*evenness* (increase)"=4))+
        scale_starshape_manual(values = c("Non significant \u0394*topography*"=15,
                                          "Significant \u0394*topography*, non significant \u0394*evenness*"=28,
                                          "Significant \u0394*topography* & \u0394*evenness* (decrease)"=23,
                                          "Significant \u0394*topography* & \u0394*evenness* (increase)"=11))+
       ...
       ggsave(sprintf("%s/z_dland_vs_even_change_%s_nolab_%s_%s.pdf",save.dir,fb,plants,ra),g1[[a]],width=4,height=4,
              device = cairo_pdf)
      # 
      ggsave(sprintf("%s/z_dland_vs_even_change_%s_%s_%s.pdf",save.dir,fb,plants,ra),g1[[a]]+
      ...
      if(a==1){
        ggsave(plot=g_legend(g1[[a]]+theme(legend.position="bottom",legend.direction = "vertical")+
                               guides(fill=guide_legend(override.aes = list(starshape=15,size=3),
                                                        nrow=4))),
               sprintf("%s/legend_z_dland_vs_even_change_%s_%s_%s.pdf",save.dir,fb,plants,ra),width=16,height=4,
               device = cairo_pdf)
   ...
   mg <- g1[[3]] +
      geom_text_repel(data=function(x){x[x$Top,]},
                      min.segment.length = 0.1,
                      aes(label=target_label),size=3,
                      parse = TRUE)+theme(axis.title.x = element_blank())+
      g1[[6]] +
      geom_text_repel(data=function(x){x[x$Top,]},
                      min.segment.length = 0.1,
                      aes(label=target_label),size=3,
                      parse = TRUE)+theme(axis.title = element_blank())+
      g1[[2]] +
      geom_text_repel(data=function(x){x[x$Top,]},
                      min.segment.length = 0.1,
                      aes(label=target_label),size=3,
      ...
      geom_text_repel(data=function(x){x[x$Top,]},
                      min.segment.length = 0.1,
                      aes(label=target_label),size=3,
                      parse = TRUE)+theme(axis.title.y = element_blank())+
      g1[[4]] +
      geom_text_repel(data=function(x){x[x$Top,]},
                      min.segment.length = 0.1,
                      aes(label=target_label),size=3,
                      parse = TRUE)+theme(axis.title = element_blank())+
      plot_layout(ncol=3)
    ggsave(sprintf("%s/z_dland_vs_even_change_%s_%s.pdf",save.dir,fb,ra),mg,width=12,height=8,
           device = cairo_pdf)
    
      
    mg <- g1[[3]] +theme(axis.title.x = element_blank())+
      g1[[6]] +theme(axis.title = element_blank())+
      g1[[2]] +theme(axis.title = element_blank())+
      g1[[5]] +theme(axis.title = element_blank())+
      g1[[1]] +theme(axis.title.y = element_blank())+
      g1[[4]] +theme(axis.title = element_blank())+
      plot_layout(ncol=3)
    ggsave(sprintf("%s/z_dland_vs_even_change_nolab_%s_%s.pdf",save.dir,fb,ra),mg,width=10,height=7,
           device = cairo_pdf)
  }
  ...
  for(ra in c("perc25","median","perc75")){#ra <- "median"
    ...
    for(plants in c("Acer","Betula","Pinus","Juglans","Populus","Larix")){#plants <- "Acer"
      ...
      g1[[a]] <- ggplot(lef[which(lef$fb==fb & lef$plant %in% plants & lef$ra %in% ra),],
        ...
        geom_star(data=function(x){x[x$landscape!="Non significant \u0394*topography*",]},
                  aes(size=landscape,starshape=landscape,fill=target_p))+
        labs(x="Z-standardized \u0394*topography* \n(Prokaryotic assembly landscape)",
             y="Z-standardized \u0394*evenness* \n(Prokaryotic assembly landscape)",
             fill="Genus",
             size="Energy landscape re-organization",
             starshape="Energy landscape re-organization")+
