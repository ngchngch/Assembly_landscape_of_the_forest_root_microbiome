...
source("packages/fun_gLV_simulation_260201.R")
...
save.dir <- "05_01_gLV_simulation_Nsp80_260201"
dir.create(save.dir)
# Parameters
total_sample <- 200
rep =200
...
Alist = foreach(n=1:nrow(set), .packages = c("igraph"))%do%{
  ...
  while(inf_ab){
    ...
    test_mat = gen_dyn(30, dt=1000, A, rlist[[1]], runif(Nsp), type="gLV")
...
dynamics = foreach(n=1:nrow(set),
                   .packages = c("igraph", "tidyverse", "deSolve", "doParallel"))%dopar%{
                     ...
                     mat = gen_dyn(time=time, dt=1000, A=A, r=r_set, N0=runif(N), type="gLV") #ode(y = N0(N), times = seq(1,500), func = lv_fun, parms = list(A=A, r=runif(N)))[,-1]
                     #plt_dynamics(mat)
                     
                     #N_perturbate = mat[time*0.9,]
                     #mat_nonperturbate = gen_dyn(time=time, dt=1000, A=A, r=r_set,
                     #                            N0=N_perturbate, type="gLV")
