...
#########################################################################
save.dir <- "05_05_01_Zconv_ELA_withRA_4step_Nsp80_nonoise_260527"
dir.create(save.dir)
...
connect <- sc[r,1]
    ...
    dir.create(sprintf("%s/seed%s/conn%s",save.dir,seed,connect))
      ...
      if(any(!is.na(or_dle[,"d_land"]))){
        ...
        z_dland <- (or_dle[match(p_seq,or_dle[,"ra"]),"d_land"]-mean(dland))/sd(dland) # HF: rowMeans, apply -> mean, sd
        z_deven <- (or_dle[match(p_seq,or_dle[,"ra"]),"d_even"]-mean(deven))/sd(deven)# HF: rowMeans, apply -> mean, sd
        
        p_dland <- c()
        p_deven <- c()
        for(perc in 1:length(p_seq)){
          p_dland[perc] <- sum(or_dle[match(c(p_seq[perc]),or_dle[,"ra"]),"d_land"] <= dland)/length(dland) # HF: dland[,perc] -> dland, ncol->length
          if(or_dle[match(c(p_seq[perc]),or_dle[,"ra"]),"d_even"]>0){
            p_deven[perc] <- sum(or_dle[match(c(p_seq[perc]),or_dle[,"ra"]),"d_even"] <= deven)/length(deven)# HF: dland[,perc] -> dland, ncol->length
          }else{
            p_deven[perc] <- sum(or_dle[match(c(p_seq[perc]),or_dle[,"ra"]),"d_even"] >= deven)/length(deven)# HF: dland[,perc] -> dland, ncol->length
          }
        }
