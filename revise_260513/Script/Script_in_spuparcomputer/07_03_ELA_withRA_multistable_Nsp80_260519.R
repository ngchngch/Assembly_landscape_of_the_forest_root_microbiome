...
#n.core=8
#dir_07_02='/Users/ngch/Desktop/Now_using/data/sugaFB_ELA/revise_260513/Analysis/07_02_evaluate_multistable_dynamics_260519'
...
#########################################################################
save.dir <- "07_03_ELA_withRA_multistable_Nsp80_260519"
dir.create(save.dir)
...
#from fujita
#RcppArmadillo version 0.12.8.4.0
...
set.seed(top)
cpath0 <- list.dirs(dir_07_02)
cpath <- list.files(cpath0[grep(sprintf("top%s",top),cpath0)],
                    pattern = "community_matrix",full.names=TRUE)
cnam1 <- sapply(strsplit(cpath,"_"),function(x){paste(x[length(x)-1],
                                                     gsub(".rds","",x[length(x)]),
                                                     sep="_")})
dir.create(sprintf("%s/%s",save.dir,cnam1))

comm_list <- readRDS(cpath)
...
for(sp in 1:80){
  ...
  part <- sprintf("%s/%s/%s",save.dir,cnam1,paste0("ID",sp))
  dir.create(part)
  ...
  enmat_ns <- ab_mat[,sp]
  enmatf <- matrix(scale(ab_mat[,sp]),ncol=1)
  ...
  if(sum(enmat_ns>0)>min_bin&sum(enmat_ns==0)>min_bin){
sa <- runSA(ocmat= bin,enmat = enmatf,
              qth=qth, rep=1280, threads=n.core)
  saveRDS(sa, file=sprintf("%s/runStopA_wo%s_%s.rds",part,sp,cnam1))
  ...
  ############
    ...
    ra_perc <- quantile(enmatf[enmat_ns>0],qt_seq)
    ran_perc <- quantile(enmat_ns[enmat_ns>0],qt_seq)
...
md_sprop <- data.frame(id=sp,
  ...
  mdp <- md_sprop
  saveRDS(mdp,sprintf("%s/ELA_SSprob_diff_ID%s_%s.rds",part,sp,cnam1))
