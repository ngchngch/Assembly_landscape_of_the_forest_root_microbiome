...
// [[Rcpp::export]]
arma::mat SA( const arma::mat& ocData, const double& qTh){
// ========================================= //
// -- Initial parameter setting
const double& maxInt=100000;
const double& momtm=0.3; // fixed
...
upd = arma::mean(arma::mean(ydif)) / nlocation;
...
mm = Rcpp::mean(upds);
if(mm < minu){minu = mm; intv = 0;}else{intv++;}
// stop iteration
if(intv > 1000){break;}
}
// ========================================= //
...
// [[Rcpp::export]]
arma::mat fullSA( const arma::mat& ocData, const arma::mat& envData, const double& qTh){
  ...
  for(int tt=0; tt < maxInt; ++tt){
    ...
    // -- stop iteration
    // max elementwise (delalpahs, delbeta)
    upd = (arma::mean(arma::mean(ydif)) / nlocation + arma::mean(arma::mean(yenvdiff)) / nlocation) / 2;
    // update upds (max len = 1000)
    if(upds.length() >= 100){
    upds.erase(0);
    }
...
// Convert to string
inline int convStr(const arma::rowvec& v, const arma::mat& ss)
{
  // Get the number of rows in the matrix ss
  int n_rows = ss.n_rows;
  
  // Compare the row vector v with each row of the matrix ss
  for (int i = 0; i < n_rows; i++) {
    // Get the i-th row vector
    arma::rowvec row = ss.row(i);
    
    // Check if the row vector v matches the i-th row vector
    if (arma::approx_equal(v, row, "absdiff", 1e-10)) {
      return i + 1;  // If they match, return the row number (1-based index)
    }
  }
  return 0;
}
...
inline double entropy(const arma::vec& v)
{
  ...
  for(int i=0; i<N; i++){
    ...
    uvec idx = find(v == uniq(i));

    double prob =
      static_cast<double>(idx.n_elem) / total;
...
// Entropy string
inline double entropy2(const Rcpp::IntegerVector& v)
{
                        ...
                        int seitr=1000, int convTime=10000)
{
  ...
  for(int i=0; i < uoc.n_rows; i++){
    ...
    for(int l=0; l < seitr; l++){
      ...
      ssid(l)=convStr(ysim,ss);
      stable(l, 0)=tt;
      stable(l, 1)=(tt==convTime);
      
    }
    
    entropyres(i,0)=entropy2(ssid);
    entropyres(i,1)=mean(stable.col(0));
    entropyres(i,2)=mean(stable.col(1));
  }
...
// [[Rcpp::export]]
arma::mat SSestimate_given(arma::rowvec alpha, arma::mat beta, arma::mat Rmat){
